...
const L = {
  en:{dir:'ltr',lang:'en',home:'/',hotels:'/hotels',cars:'/cars',esim:'/esim',comp:'/compensation',nav:{flights:'Flights',hotels:'Hotels',cars:'Cars',esim:'eSIM',dest:'Destinations',blog:'Blog',comp:'Compensation'},
      from:'from',perRoute:'cheapest fare found now',search:'Search this route →',airlines:'Airlines on this route',flighttime:'Flight time',direct:'often direct',
      ...
      note:'Live starting fare from our partners; final price is set by the airline. We may earn a commission at no extra cost to you.',secure:'Secure & free to use · No booking fees'},
  fr:{dir:'ltr',lang:'fr',home:'/fr/',hotels:'/hotels',cars:'/cars',esim:'/esim',comp:'/compensation',nav:{flights:'Vols',hotels:'Hôtels',cars:'Voitures',esim:'eSIM',dest:'Destinations',blog:'Blog',comp:'Indemnisation'},
  ...
  ar:{dir:'rtl',lang:'ar',home:'/ar/',hotels:'/hotels',cars:'/cars',esim:'/esim',comp:'/compensation',nav:{flights:'رحلات',hotels:'فنادق',cars:'سيارات',esim:'eSIM',dest:'الوجهات',blog:'المدوّنة',comp:'تعويضات'},
...
function tips(f,t,lang){var c=nm(t,lang);var dom=isDom(f,t);
  if(lang==='en')return dom
    ? 'Domestic fares to '+c+' are usually low but rise on weekends and holidays. For the cheapest seats, fly mid-week and book a couple of weeks ahead. Use the live search above for your exact dates.'
    : 'Fares to '+c+' spike during summer and Eid, when the diaspora travels. For the lowest prices, fly mid-week (Tue/Wed) and off-season, and book roughly 4–6 weeks ahead. Use the live search above to see the cheapest dates.';
...
function faqs(f,t,lang){
  ...
  var q=[];
  if(lang==='en'){
    q.push(['How much is a flight from '+o+' to '+c+'?','Fares start from the live price shown above and change daily. Compare all airlines in one search to grab the lowest fare for your dates.']);
...
function head(f,t,lang){
  ...
  return '<!DOCTYPE html>\n<html lang="'+L[lang].lang+'" dir="'+L[lang].dir+'">\n<head>\n<meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" />\n'
    ...
    +'<meta property="og:title" content="'+title(f,t,lang)+'" /><meta property="og:description" content="'+metaDesc(f,t,lang)+'" /><meta property="og:url" content="'+self+'" /><meta property="og:type" content="website" />\n'
...
// ---- sitemap ----
var urls=['/','/hotels','/cars','/esim','/compensation','/flight-rights','/fr/flight-rights','/ar/flight-rights','/about','/contact','/privacy'].map(function(p){return 'https://oasisdeal.com'+p;});
...
function fullFooter(lang){
  ...
  var explore=col(T.ex,[[H,T.flights],['/hotels',T.hotels],['/cars',T.cars],['/esim',T.esim],['/compensation',T.comp],[P+'/destinations/',T.dest],[P+'/blog/',T.blog]]);
